...
export function GeoJsonPanel({ featureCollection, onImport }: GeoJsonPanelProps) {
  ...
  return (
    <Card className="h-full p-4">
      ...
      <Tabs defaultValue="viewer" className="h-[calc(100%-28px)]">
        <TabsList>
          <TabsTrigger value="viewer">Viewer</TabsTrigger>
          <TabsTrigger value="editor">Editor</TabsTrigger>
        </TabsList>
        <TabsContent value="viewer" className="h-[calc(100%-74px)]">
          <pre className="h-full overflow-auto rounded-md border border-border bg-slate-50 p-3 text-xs leading-6">{formatted}</pre>
        </TabsContent>
        <TabsContent value="editor" className="space-y-3">
          <textarea
            ...
            className="h-[420px] w-full rounded-md border border-border p-3 font-mono text-xs"
          />
        </TabsContent>
      </Tabs>
      <div className="mt-3 flex gap-2">
        <Button variant="outline" onClick={() => onValidate(editorValue)}>Validate</Button>
        <Button onClick={onDownload}>Download .geojson</Button>
      </div>
    </Card>
  )
}
